...
- **Before writing "the text says X" about an English word, check what X is translating.** `bible_interlinear <book> <ch> <v>` (or `query.py interlinear`) gives unfoldingWord ULT's word-level alignment to the Hebrew and Greek — the only link in this database between an English word and the original it renders, and the cheapest guard there is against building a point on a translator's choice. Two things to hold: the mapping is many-to-many by nature (Genesis 1:1's "the heavens" renders both אֵת and הַשָּׁמַיִם, since the object marker has no English of its own), and the Greek side is the UGNT, not this project's default SBLGNT. The two differ in about one verse in six by raw count, but that number oversells it: most of the gap is manuscript orthography (Δαυείδ for Δαυίδ, Πειλᾶτος for Πιλᾶτος), and only 87 verses in the whole New Testament differ by three or more words. Where UGNT *does* diverge substantively it is worth having — Mark 16:8ff, the pericope adulterae, Luke 23:34, and John 1:34, where it reads υἱός against SBLGNT's ἐκλεκτός. So use it as a genuine second witness rather than discounting it, and confirm with `bible_verse` which text a reading came from before resting a New Testament argument on the wording.
...
- **Consult commentaries last**, not first — use them to check your reading, not to form it. Cite any extra-biblical source used, per AGENTS.md.
  ...
  - **There is now an open-licence footnote corpus in `bible-text.db` too, so this habit no longer depends on the external volume being mounted.** 1,297 notes from unfoldingWord: `uw-uhb` 949 (930 of them **Qere** readings, `note_type='qere'` — UHB prints the Ketiv and footnotes the Qere), `uw-ult` 326 translator decisions, `uw-ugnt` 22 textual notes on omitted passages. They arrive automatically on `bible_verse` and on `query.py passage --notes`, resolved across versification schemes, so an English-verse lookup surfaces the Hebrew note attached to it. Ruth 2:1 shows the pair working: ULT records the fork and UHB gives the Qere behind it. Read these the same way as the commercial footnotes above — the translators telling you where the text is genuinely ambiguous.
...
- **A long study earns two more sections, both from [structural-readability.md](../read-bible-study/structural-readability.md) rules 10-11.** At 2,500+ words and 6+ `##` sections — 38 of 113 pages in this corpus — add a **Study outline** straight after Key Takeaways: a short annotated list of the major sections, a briefing rather than a repeat of the table of contents mkdocs already generates. Its value is the annotations, the grouping of a run of sections into one line, and telling the reader where they are standing. Write descriptive annotations and leave evaluative ones to the author. And put apparatus — source-weighting, correctives, a source underwriting several sections at once — in a single `## Annex:` after the last teaching section, keeping Discussion Questions with the teaching they examine.
...
- **Key Takeaways** replaces the old flat "Key lessons" list with up to five subheadings **in this order**, each included only where the study earns it — don't force one to fill a slot. Rationale: [key-takeaways.md](../../../docs/content/about/key-takeaways.md). The section comes up front, before supporting detail — a reader should get the point even if they read no further.
  ...
  | **Memory verses** | 1–3 verses *already quoted and cited in the study's own body*, pointed back to — not new quotations introduced here. Block-quote shape, reference linking to Blue Letter Bible (verify the link resolves for the translation quoted), no explanatory sentence after the verse. A bare "See [section]" locator is fine in a study with several parallel sections; an interpretive sentence is not — it belongs in Lessons about Jesus or the walk-through instead. |
  | **Be Transformed** | Romans 12:2 made concrete, as three labeled bullets — `**Think.**` (a thought or belief to correct), `**Attitude.**` (a posture to adopt), `**Do.**` (an action to take). This is the default shape, not one option among several; a different shape is a deliberate choice for a study whose application genuinely won't split three ways, not a fallback for when the three bullets are more work than a paragraph. Keep it as distinguishable from the exegesis as Phase 5 already requires. |
...
- **When you hand a draft back, hand back [human-review.md](human-review.md) with it.** Three things
  survive every automated pass — the antithesis construction, essay register in the applying
  sections, and writing around the theology instead of naming it. All three were found by the site's
  author correcting a study by hand after it had passed two adversarial reviews and a clean
  validate. Say which of the three you think the draft is weakest on.
...
- **Write toward [style-guide.md](style-guide.md)'s "Write toward this" section from the first draft, and re-run the whole guide after every substantive revision — not only before flipping `draft: false`.** The five positive rules there (every sentence adds a fact, a citation or a step; name the actor and show the mechanism; lead with the fact; let the citation carry the emphasis; define affirmatively rather than by contrast) prevent the whole class of AI-register tics; **rule 5 is a structural prohibition to apply before drafting, not a cleanup pass** — thesis-antithesis, dialectical hedging and contrastive reframes are ruled out at the point of writing, because that family does not respond to detection; the diagnostic half of that page only catches instances after the event. **Revision is where the register is produced, not first drafting** — on `last-things/day-is-a-thousand-years.md` four narrated-argument constructions were cut in one pass and three more appeared in the next two revisions of the same paragraphs, written within the hour by the same agent that had just removed them. A green `npm run validate` is not evidence this pass happened: its four style checks cover `worth ___`, the "rather than" virtue contrast, bullet length and reader-reassurance, and nothing else on that page.
...
- [human-review.md](human-review.md) — the three things only a human read-through catches (hand to the user with the draft)

...
Phase 4 of [SKILL.md](SKILL.md) links here. Use the full method below for words that are load-bearing or ambiguous in context (Paul's "righteousness" in Rom. 1:17, "flesh" in Rom. 8, etc.) — not for every noun that happens to have a Strong's number. For a routine term, the lighter treatment already in Phase 4 (Strong's number, gloss, pronunciation) is sufficient; reaching for the full apparatus below on a word whose meaning isn't actually in question is padding, not rigor.

## Step 1 — Diachronic: how the word arrived here

Trace the word's usage *through time*, in order: Classical Greek, then the Septuagint (LXX — the Greek OT Paul and the early church actually read), then other Hellenistic Jewish literature (apocrypha, pseudepigrapha, rabbinic sources, Qumran/Dead Sea Scrolls), then — for Greek terms specifically — the documentary papyri (private letters, receipts, contracts; Moulton & Milligan's *Vocabulary of the Greek Testament*, see [references/README.md](../../../references/README.md#word-study--original-language-tools)). The papyri matter because they're *not* literary or theological writing — they show how a word was used in ordinary first-century speech, which is a check against reading a NT term as more technical or "special" than it actually was outside a theological text. The goal throughout is a *range* of attested meanings, not a single "root meaning."

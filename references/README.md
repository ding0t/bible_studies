...
A third script, `references/build/commentary_index.py`, is different in kind from the two databases above — it doesn't ingest any external source. It scans the `primary_passage` and `bible_references` frontmatter of this repo's own subject sections under `docs/content/` (the allow-list in the script's `SUBJECT_DIRS`) and maintains an auto-generated cross-reference section inside `docs/content/commentaries/<NN>-<book>/` (a chapter file and a book `index.md` per referenced book/chapter), so a reader looking at a commentary page can see which studies actually treat that passage. It writes directly into committed content — run it after adding or editing a study's reference frontmatter (`uv run python commentary_index.py`). It's idempotent and only ever touches the content between `<!-- commentary-index:auto-start -->` / `...auto-end -->` markers, so hand-written commentary prose around that section is never overwritten. See the develop-bible-study skill's Phase 7 for why every study should populate `primary_passage`/`bible_references` — a study missing both is simply invisible to this index, not an error.
...
| Semantic domains (Louw-Nida / SDBH) | `open-data/macula-greek`, `open-data/macula-hebrew` | open (one field caveat — see [docs/content/resources/public-data-sources.md](../docs/content/resources/public-data-sources.md)) | Cite freely |
...
### The source catalog (`references/sources.toml`)

One file holds three facts that used to live in three places that could disagree: **where** each
source is, **what tier** it sits in, and **how much** of it may be reproduced. `media_root.py`
(locations), `build.py` (licence string → tier), `study_notes_query.py` (quote allowance) and
`check_sources.py` (drift) all read it through `references/build/source_catalog.py`. Read it that
way too — a second parser is exactly how the three drifted apart before.

`check_sources.py` now fails if the catalog's raw-only list disagrees with what `build.py`
actually ingests, or if anything claims a tier `[tiers]` doesn't define.

`sources.toml` also carries a **profile per source** — `kind`, `edition`, `era`, `purpose`, `strengths`, `limits` — covering all 320 works. The licence tiers answer *may I quote this?*; profiles answer *should I be using this here, and what does it not tell me?*, which previously existed only as prose in [docs/content/scripture/translations.md](../docs/content/scripture/translations.md). The `edition` vocabulary (manuscript / diplomatic / eclectic / derived / translation) is taken verbatim from that page rather than invented here. There is deliberately no primary/secondary/tertiary field: that ranking is a property of the question, not the source. `check_sources.py` fails on a profile with an undefined kind or edition, on a text-bearing source with no edition, and on any profile recording no `limits`. Read them with `source_catalog.profile_for(work_id)` or the `source_profile` MCP tool.

It is TOML, against this repo's YAML habit, for one concrete reason: `check_sources.py` is
documented as runnable from a bare `python3` with no `uv sync`, and PyYAML is not importable there
while `tomllib` is. `license_map.yml` was folded into its `[license_map]` table on 2026-09-18,
verbatim and verified identical.
...
The catalog's `[license_map]` (`references/sources.toml`) tiers BY-SA as `open`, which is right for quoting and for a gitignored local
...
`references/build/mcp_server.py` exposes the same lookups as MCP tools (`bible_word`, `bible_concordance`, `bible_domain`, `bible_verse`, `bible_syntax`, `bible_passage`, `bible_crossref`, `bible_align`, `bible_parallel`, `bible_quotations`, `review_gaps`, `bible_works`, `twot_root`, `twot_strongs`, `twot_lemma`, plus the `study_*` family over study-notes.db — `study_status`, `study_verse`, `study_note`, `study_intro`, `study_article`, `study_works` — `research_batch_run`/`research_batch_tools`, `passage_brief`, `evidence_draft`/`evidence_check`, `source_profile`, and `read_structure`), registered project-wide via `.mcp.json` at the repo root. It reports its own version in `serverInfo` (from `references/build/pyproject.toml`); leaving that unset made it report the `mcp` SDK's version instead, which moves on an SDK upgrade and never moves when a tool changes. **Other hosts keep their own config, and there are two, with different schemas.** LM Studio reads `~/.lmstudio/mcp.json` (an `mcpServers` object, same shape as this repo's `.mcp.json`). Bionic reads `~/.lmstudio/apps/bionic/.internal/ng-mcp.json` — a `servers` **array**, each entry `{id (uuid4), name, enabled, connection: {type, command, args, env}}`. Registering in the first does **not** register in the second. In both, the command must be an absolute `uv` path and an absolute `--directory`: a GUI app inherits neither shell `PATH` nor a working directory. Neither host rereads its config while running, so restart after editing. It is a thin wrapper, not a second implementation: every tool calls a `lookup_*` function imported straight from `query.py` or `twot_lookup.py`, so the CLI and the MCP server can never return different answers to the same question, and both scripts keep working from the terminal (or from an agent's Bash tool as a fallback) whether or not the MCP server is configured. Adding a new lookup means adding one function to `query.py`/`twot_lookup.py` plus one `@mcp.tool()` wrapper — no query logic belongs in `mcp_server.py` itself. `study-notes.db` is wired in through `study_notes_query.py` (added 2026-09-18), which follows the same library-plus-thin-wrapper rule but adds three things the other tiers don't need: it **refuses queries that would scan** (a full scan of it took 60–100s and read to an agent as a hung volume back when it was NAS-mounted over SMB, before the 2026-09-19 move to local disk — still a needless full scan now it's fast), it stamps every returned record with its tier and attribution rather than relying on a separate `works` call, and it answers `{available: false, reason, remedy}` instead of raising when the external volume is absent. Snippet caps bound what one call returns; they are not a limit on lookup, since the `quotation-only` tier governs reproduction in a committed file, not consultation.

**`research_batch_run` is the preferred entry point once a task needs more than two or three lookups.** It dispatches to the same `lookup_*` functions — a test asserts every registry entry resolves to `query`, `study_notes_query` or `twot_lookup`, so a batch answer and a single-tool answer cannot diverge — but shares one connection per database instead of reopening for each call. Measured across twelve `study_note` lookups: 1.27s individually, 0.14s batched. It also enforces a wall-clock budget through a SQLite progress handler, so a query that would scan is abandoned and returned as `timed_out` rather than hanging the call; and every request carries its own status, so one unreachable source degrades to `unavailable` for that request alone rather than failing the batch.

**`passage_brief` is the entry point for starting on a passage.** It composes registered batch lookups — a test asserts every request it builds is a tool in `research_batch.REGISTRY`, so it cannot become a second implementation — and returns them in the develop skill's exegesis order. Two things it does that a hand-assembled set of calls reliably does not: it computes **versification unconditionally and puts it first**, flagging a reference whose numbering differs across traditions (the Joel 2:28 / Joel 3:1 class, which raises no error and so goes unnoticed); and it routes a translation name to whichever database actually holds it, so asking for the ESV returns the ESV rather than an empty result from the open database. Per-verse detail is capped at 12 verses and says when it truncated.
...
Git submodules of forked open-data repos (Bible texts, lexicons, morphology, cross-references) — see [docs/content/resources/public-data-sources.md](../docs/content/resources/public-data-sources.md) for the full master list, license findings, and why each one's there. Also see that doc's **eBible.org section** for translations fetched at build time rather than forked (not git-hosted, so the submodule pattern doesn't apply).
...
For Hebrew *language learning* (as opposed to word-study lookup), see the existing [resources/hebrew-learning-resources.md](../docs/content/resources/hebrew-learning-resources.md).
...
- *How to Read the Bible for All Its Worth* (Fee & Stuart, 4th ed.) — the methodology behind the develop-bible-study skill. Source PDF + a full local markdown extraction live next to each other on the media volume, for personal reference only. The site's own public write-up of these principles, for readers rather than for the skill tooling, is [docs/content/scripture/how-to-read-the-bible.md](../docs/content/scripture/how-to-read-the-bible.md).

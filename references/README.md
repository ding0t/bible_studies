...
These are commercially-sold or otherwise copyrighted works. The **source files themselves** aren't committed to this repo — this is a public repo, and committing an entire copyrighted work would be redistribution, not personal use. Each one's source file lives only on the personal media volume that owns it.
...
- **TWOT** (*Theological Wordbook of the Old Testament*, Archer, Harris & Waltke, Moody Publishers) — Hebrew word-study entries (root, cognates, theological discussion), the standard companion to Strong's/BDB for Hebrew. Source PDF on the media volume; this repo has a partial extraction pipeline at `references/build/twot/` (OCR + segmentation, in progress — see `PLAN_derivative_structure.md` there for status). `references/build/twot/twot_strongs_map.json` **is committed** and gives a lightweight `TWOT root → {strongs_id, bdb_id, lemma, xlit, gloss}` lookup usable right now (bare identifiers/glosses only, not TWOT's descriptive prose — see the caveat already on the `strongs` fork in [docs/content/resources/github.md](../docs/content/resources/github.md), same distinction applies here: numbers/glosses fine, discussion text is TWOT's own copyrighted prose).

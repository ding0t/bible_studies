...
What was wrong was treating it as a blocker: the repos clone fine, and all four are now submodules
pointing at Door43 directly.

**One weakening of the pattern, recorded rather than hidden.** Every other source here is forked to
`ding0t/*` first, so the project survives an upstream disappearing. These four are not: `gh auth`
could not authenticate from the machine that added them, so no mirror was created. The submodules
pin exact commits, which protects against upstream *changing*, but not against Door43 going away.
Mirroring them is worth doing when the auth is working.

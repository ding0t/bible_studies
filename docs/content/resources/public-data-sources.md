...
| [openscriptures/morphhb](https://github.com/openscriptures/morphhb) | Westminster Leningrad Codex (WLC) + morphological/lemma tagging | WLC text: public domain. Tagging: CC BY 4.0 | ✅ Used | The Hebrew source this site's own word studies are required to work from. |
...
| [unfoldingWord/en_uhg](https://git.door43.org/unfoldingWord/en_uhg) (UHG) | Reference grammar of biblical Hebrew, 96 articles keyed to the same morphology codes | CC BY-SA 4.0 (confirmed in `LICENSE.md`, 2026-09-05) | ✅ Used | Tiny (~3MB), `.rst` files by category — *adjective_gentilic*, *definiteness*, *conjunction*. Fills a real gap rather than duplicating: this project holds lexicons (BDB, Strong's, TWOT) and morphology tags but **no grammar**, while This site's own studies are required to ground grammar claims rather than recall them, and until this was taken there was nothing to ground them in. |
...
| Source | What it is | Licence | Used here? | Why it matters |
|---|---|---|---|---|
| [openscriptures/HebrewLexicon](https://github.com/openscriptures/HebrewLexicon) | BDB (Brown-Driver-Briggs) outline linked to Strong's | CC BY 4.0 (BDB/Strong's underlying content itself is public domain) | ✅ Used | The primary Hebrew lexicon behind the word studies on this site. |
...
| [Clear-Bible/macula-greek](https://github.com/Clear-Bible/macula-greek) | Per-word Greek NT (SBLGNT) morphology, lemma, gloss, and **Louw-Nida semantic domain codes** | CC BY 4.0 (Biblica, confirmed in `LICENSE.md`). Underlying sources (Nestle1904, SBLGNT, Berean Interlinear, Cherith glosses) are all PD or CC BY 4.0. **Caveat:** the Louw-Nida domain field (`@ln`, `@domain`) is drawn from UBS's MARBLE project "used with permission" **to Clear-Bible specifically** per their own license notes — not itself a blanket CC grant. Cite carefully, same spirit as the TWOT caveat on the `strongs` fork above. | ✅ Used | Turns the print-only Louw & Nida citation in the this site's word-study method into a queryable local source. |
| [Clear-Bible/macula-hebrew](https://github.com/Clear-Bible/macula-hebrew) | Per-word Hebrew OT (WLC) morphology, lemma, gloss, and **SDBH semantic domain codes** — the Hebrew-side equivalent of Louw-Nida, which is Greek-only | CC BY 4.0 (Biblica, confirmed in `LICENSE.md`) | ✅ Used | |
...
## Others in this space, not yet assessed

Worth knowing about, and not yet examined closely enough for this page to say anything about their licensing. Listed so nothing is quietly forgotten.
...
Internet Archive instead: public-domain critical editions — CSEL, GCS, Migne — as scanned text.
...
Irenaeus, Augustine and Tertullian — each pulled for a specific passage rather than collected. [Patristic Sources](patristic-sources.md) sets out which question each was fetched for, and what was
checked inside it before it was kept.

## eBible.org (not GitHub — fetched at build time, not forked)

[eBible.org](https://ebible.org/find/) is a plain file host rather than a code repository, so its texts are fetched directly and pinned to a recorded version. It is where several of the public-domain translations behind this site come from.

| eBible.org id | What it is | License | Notes |
|---|---|---|---|
| `eng-web` | World English Bible (WEB) — full Bible | Public Domain (confirmed on-page) | This site's default English translation, previously not actually present (scrollmapper's bundle only has the unrelated older "Webster"/"RWebster" PD translations, despite a stray empty `WEB.db` stub suggesting otherwise). |

...
| **UHB** — unfoldingWord Hebrew Bible | 2022– | The same Westminster Leningrad Codex, re-encoded in USFM from the Open Scriptures Hebrew Bible, with per-word lemma, Strong's and morphology | The text the ULT's Old Testament alignment resolves against, which is its real job here. Carries **949 footnotes — 930 of them Qere readings**, the Masoretes' own "the consonants say X, read Y", which arrive automatically on any verse lookup. Openly licensed (CC BY-SA 4.0) | **Not a second witness to the Hebrew** — it is the same WLC consonantal text, differing genuinely in only ~1.5% of verses, and for a stated reason: "in order to avoid subjectivity, the text of the UHB uses the [Ketiv](../glossary.md#ketiv-qere) of the WLC", where the WLC prints the Qere. And it uses a different [versification](../glossary.md#versification) — unfoldingWord's own note says it follows the ULT's scheme rather than "the WLC scheme common for Hebrew Bibles", which "may make some resources that are keyed to the WLC more difficult to use". That is not theoretical: the same reference names a different verse in `uw-uhb` and `morphhb-wlc` across ~1,500 verses in Joel, 1 Chronicles, 1 Kings, Numbers, Job, Ezekiel and Malachi. Compare the two with `bible_parallel`, never by reference alone | ✅ `uw-uhb` |
...
| **UGNT** — unfoldingWord Greek New Testament | 2022– | The Greek text unfoldingWord's translations are aligned to, following the *Bunning Heuristic Prototype* | Openly licensed (CC BY-SA 4.0) and queryable here; a fourth lineage alongside SBLGNT, Tischendorf and the Byzantine text, and the text that makes ULT's word-level alignment resolvable | **A different kind of edition** — a heuristic text from the Center for New Testament Restoration rather than a committee edition like NA28. It differs from SBLGNT in roughly one verse in six by raw count, but most of that gap is manuscript spelling rather than a different text — Δαυείδ for Δαυίδ, Πειλᾶτος for Πιλᾶτος — and only 87 verses in the whole New Testament differ by three or more words. Those that do fall at the well-known cruxes: the ending of Mark, the woman taken in adultery, Luke 23:34, and John 1:34, where it reads Υἱὸς against SBLGNT's ἐκλεκτός. Check which text a reading comes from before resting an argument on it | ✅ `uw-ugnt` |
